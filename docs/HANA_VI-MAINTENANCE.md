...
```
E:\SPT 4.1\SPT_Runtime\user\mods\HANA_VI-AIO\
  config.json                  ← 패치 42개 on/off
  db\ammo.json                 ← 구경별 탄약 묶음
  db\patches\01_ar_firerate_change.json   ← 패치 내용 (여기가 본체)
  db\patches\02_smg_firerate_change.json
  ...                          (42개)
  db\locales\global\ko.json    ← 아이템 이름/설명

E:\SPT 4.1\SPT_Runtime\user\mods\HANA-VI-AllExamined\
  config.json
```
