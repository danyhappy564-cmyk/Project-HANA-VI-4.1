...
class Mod implements IPostDBLoadMod {
    ...
    mod: string
    private wttTraderIds: Record<string, string> = {
    ...
    private wttCurrencyIds: Record<string, string> = {
    ...
    // AUG 파츠 호환성 패치
    private addAugCompatibility(tables: any): void
    {
    ...
    private injectMxlr(tables: any, modPath: string): void
    {
        ...
        if (fs.existsSync(itemsPath))
        {
            ...
            const mxlrItems = JSON.parse(itemsStr) as Record<string, any>;
        ...
        if (fs.existsSync(handbookPath))
        {
            ...
            const mxlrHandbook = JSON.parse(handbookStr) as { Items?: any[] };
        ...
        if (fs.existsSync(localesDir))
        {
            ...
            for (const file of localeFiles)
            {
                ...
                const locData = JSON.parse(localeStr) as Record<string, string>;
                ...
                {
                    (gameLocale as any)[key] = value;
                }
            }
        }
    }

    private injectSdtacKits(container: DependencyContainer, tables: IDatabaseTables, modPath: string): void
    {
        // 공용 logger / jsonUtil
        ...
        if (this.mydb.items)
        {
            for (const [ID, Item] of Object.entries(this.mydb.items as Record<string, any>))
            {
                if (!Item || !Item.enable)
                {
                    continue;
                ...
                if ("clone" in Item)
                {
                    this.cloneItem((Item as any).clone, ID);
                    this.copyToFilters(
                        (Item as any).clone,
                        ID,
                        Item.enableCloneCompats,
                        Item.enableCloneConflicts
                    );
                }
                else
                {
            ...
            {
                const item = (this.mydb.items as any)[ID];
                if (item && item.enable)
                {
        ...
        {
            for (const [ID, Article] of Object.entries(this.mydb.clothes as Record<string, any>))
            {
                if (!Article)
                {
                    continue;
                ...
                if ("clone" in Article)
                {
                    this.cloneClothing((Article as any).clone, ID);
                }
                else
                {
                ...
                // 의류 로케일
                this.addLocales(ID, undefined, Article as any);
            }
    ...
    private injectSigMcxVirtus(container: DependencyContainer, tables: IDatabaseTables, modPath: string): void
    {
    ...
    // MCX 파츠 호환성 패치
    private addMCXCompatibility(tables: any): void
    {
        const items = tables.templates.items;
    ...
    private injectCarlQhb(container: DependencyContainer, tables: IDatabaseTables, modPath: string): void
    {
        // 공용 logger / jsonUtil
        this.logger = container.resolve<ILogger>("WinstonLogger");
        this.jsonUtil = container.resolve<JsonUtil>("JsonUtil");
    ...
    private injectAtlasGear(container: DependencyContainer, tables: IDatabaseTables, modPath: string): void
    {
        // 공용 logger / jsonUtil
        this.logger = container.resolve<ILogger>("WinstonLogger");
        this.jsonUtil = container.resolve<JsonUtil>("JsonUtil");
        ...
        if (this.mydb.custom_items)
        {
            ...
            for (const [customKey, customItem] of Object.entries(this.mydb.custom_items as Record<string, any>))
            {
                if (!customItem)
                {
                    continue;
                ...
                const newId = (customItem as any).sptID ?? customKey;
                (this.mydb.items as any)[newId] = customItem;
            }
        }
        ...
        if (this.mydb.custom_clothes)
        {
            ...
            for (const [customKey, article] of Object.entries(this.mydb.custom_clothes as Record<string, any>))
            {
                if (!article)
                {
                    continue;
                ...
                const newId = (article as any).sptID ?? customKey;
                (this.mydb.clothes as any)[newId] = article;
            }
        }
...
private injectQbz191(container: DependencyContainer, tables: IDatabaseTables, modPath: string): void
    {
    ...
    private checkItem(itemToCheck: any): [boolean, ITemplateItem]
    {
    ...
    private processWttInventorySlots(tables: IDatabaseTables, itemId: string, itemConfig: any): void
    {
        // 설정에 슬롯 정보가 없으면 스킵
        ...
        for (const slot of defaultInventory._props.Slots as any[])
        {
            ...
            const filterArray = filters[0].Filter as string[];
    ...
    */
    ...
    private addLocales(ID: string, Item?: any, Article?: ICustomizationItem): void
    {
        const name =            ID + " Name";
    ...
    private loadWttCombinedConfig(modPath: string): { [itemId: string]: any }
    {
        ...
        const combined: { [itemId: string]: any } = {};
        ...
        for (const file of configFiles)
        {
            ...
            try
            {
                const parsed = JSON.parse(raw) as { [itemId: string]: any };
    ...
    private createWttCloneItem(itemConfig: any, itemId: string): NewItemFromCloneDetails
    {
        ...
        const cloneDetails: NewItemFromCloneDetails = {
    ...
    private processWttTraders(tables: IDatabaseTables, itemConfig: any, itemId: string): void
    {
        ...
        const traderKey: string = itemConfig.traderId;
        const finalTraderId: string = this.wttTraderIds[traderKey] || traderKey;
    ...
    private injectWttCustomItems(container: DependencyContainer, tables: IDatabaseTables, modPath: string): void
    {
    ...
    public postDBLoad(container: DependencyContainer): void {
        ...
        this.injectWttCustomItems(container, tables as IDatabaseTables, modPath);

        // Inject NW-QBZ191 content
        this.injectQbz191(container, tables as IDatabaseTables, modPath);

        // Inject Carl-QHB content (from 기존 mod.ts)
        this.injectCarlQhb(container, tables as IDatabaseTables, modPath);

        // Inject SIG_MCX_VIRTUS content (from 기존 mod.ts)
        this.injectSigMcxVirtus(container, tables as IDatabaseTables, modPath);

        // Inject SDTAC KITS (MFACitems) content
        this.injectSdtacKits(container, tables as IDatabaseTables, modPath);

        // Inject ATLAS-GEAR content
        this.injectAtlasGear(container, tables as IDatabaseTables, modPath);
        ...
        if(this.cfg.Extended_mags)
        {
            ...
            const magCompatMap: [string, string][] = [
...
function loadItemidsForLang(localesRoot: string, langId: string): Record<string, Record<string, string>> | null
{
...
function applyMosinExtension(container: DependencyContainer): void
{
    const databaseServer = container.resolve("DatabaseServer") as any;
    const tables = databaseServer.getTables();
    const items = tables.templates.items;
    const handbook = tables.templates.handbook;
    const itemHelper = container.resolve("ItemHelper") as any;
    ...
    {
        const newitems = JSON.parse(fs.readFileSync(newitemsPath, "utf-8")) as any;
    ...
    {
        const modifyItem = JSON.parse(fs.readFileSync(modifyItemPath, "utf-8")) as any;
        ...
        if (Array.isArray(modifyItem.itemsToAdd))
        {
            ...
            {
                const addId = modSlots._id as string;

                // 슬롯 추가
                if (modSlots.slotAdd)
                {
                    modSlots.slotAdd.targetId.forEach((value: string, index: number) =>
                    {
                        ...
                        const slot = baseItem._props.Slots.find((s: any) => s._name === slotName);
                ...
                {
                    const traderId = modSlots.traderAssort.traderName as string;
                    ...
                    const loyalValue = Object.values(itemList.loyal_level_items)[0] as number;
        ...
        if (Array.isArray(modifyItem.itemsToConflicts))
        {
            for (const modConflts of modifyItem.itemsToConflicts)
            {
                ...
                for (const value of modConflts.ConflictingItems as string[])
                {
    ...
    if (enItemids)
    {
        const allLocales = Object.values(tables.locales.global) as Record<string, string>[];

        for (const locale of allLocales)
        {
            for (const [idIndex, idName] of Object.entries(enItemids))
            {
                for (const [des, value] of Object.entries(idName as Record<string, string>))
                {
                    locale[`${idIndex} ${des}`] = value;
                }
            }
    ...
    for (const langId of availableLangs)
    {
        ...
        const targetLocale = tables.locales.global[langId] as Record<string, string>;
...
interface G36Injection
{
    overwriteProperties: {
        ...
        overrideProperties?: Record<string, any> | null;
    };
    slotsToAdd?: {
        targetID: string[];
        slotName: string[];
    ...
    traderToAdd?: {
        ...
        barter_scheme: "ROUBLES" | "DOLLARS" | "EUROS" | string;
        barter_scheme_value: number;
        loyal_level_items: number;
    };
}

function loadG36InjectionFiles(nervRoot: string): any[]
{
    const results: any[] = [];
...
function isCorrectG36Injection(obj: any): obj is G36Injection
{
    return obj
...
function processG36InjectionItemTemplate(
    customInjection: G36Injection,
...
): void
{
    ...
    if (customInjection.slotsToAdd)
    {
        ...
        {
            const baseItem = items[value];
            if (!baseItem?._props?.Slots) return;
    ...
    if (customInjection.traderToAdd?.traderID)
    {
        ...
        //  - DOLLARS: 5696686a4bdc2da3298b456a
        //  - EUROS  : 569668774bdc2da2298b4568
        let traderCurrencyTpl: string = customInjection.traderToAdd.barter_scheme;
...
function applyG36Extension(container: DependencyContainer): void
{
    const databaseServer = container.resolve("DatabaseServer") as any;
    const tables = databaseServer.getTables();
    const items = tables.templates.items;
    const jsonUtil = container.resolve("JsonUtil") as any;
...
function applyTT33KExtension(container: DependencyContainer): void
{
    const jsonUtil = container.resolve("JsonUtil") as any;
    const databaseServer = container.resolve("DatabaseServer") as any;
    const tables = databaseServer.getTables();
    ...
    function addPreset(preset: any, makingChangeOnExistingPreset = false): void
    {
    ...
    const tt33kmid: string[] = [];
    const tt33kgmid: string[] = [];
    const type54mid: string[] = [];
    for (let i = 0; i < 5; i++)
    {
    ...
    const questTemplates = tables.templates.quests as Record<string, any>;
    ...
    for (const localeID of availableLangs)
    {
        ...
        const targetLocale = tables.locales.global[localeID] as Record<string, string>;
        for (const [idIndex, idName] of Object.entries(langItemids))
        {
            for (const [des, value] of Object.entries(idName as Record<string, string>))
            {
...
// ---------------- Nervex(ShAK-12-Sys) 통합 ----------------

interface NervexInjection
{
    overwriteProperties: {
        newId: string;
        itemTplToClone: string;
...
function loadNervexInjectionFiles(nervRoot: string): any[]
{
    const results: any[] = [];
...
function isCorrectNervexInjection(obj: any): obj is NervexInjection
{
    return obj && obj.overwriteProperties !== undefined;
}

function processNervexInjectionItemTemplate(
    customInjection: NervexInjection,
    items: Record<string, any>,
    jsonUtil: any,
    tables: any
): void
    ...
    if (customInjection.overwriteProperties)
    {
        ...
        if (override && typeof override === "object")
        {
            for (const [key, value] of Object.entries(override))
            {
                ...
                {
                    console.log(
                        "[Nervex] Key %s not found in item %s",
                        key,
                        customInjection.overwriteProperties.itemTplToClone
                    );
                }
            }
...
function applyNervexExtension(container: DependencyContainer): void
{
    const databaseServer = container.resolve("DatabaseServer") as any;
    const tables = databaseServer.getTables();
    const items = tables.templates.items;
